...
impl<'a> Client<'static> {
    ...
    async fn send_message(sender_outgoing: &Sender<String>, msg: json_rpc::Message) {
        ...
        println!("SEND CLIENT: {}", serde_json::to_string(&msg).unwrap());
    ...
    pub async fn send_authorize(&mut self) {
        let id = SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        if let Ok(authorize) = self.authorize(id, "name".to_string(), "pass".to_string()) {
...
impl<'a> IsClient<'a> for Client<'a> {
    ...
    fn authorize(
        ...
        match self.status() {
            ...
            _ => {
                self.sented_authorize_request.push((id, "test".to_string()));
